...
function show(i){index=Math.max(0,Math.min(state.manifest.records.length-1,i));let r=state.manifest.records[index];image.onload=()=>{fit();};image.src="/"+r.image;$("counter").textContent=`Frame ${index+1} of ${state.manifest.records.length} · ${r.split} · ${r.width} × ${r.height}`;$("next").style.visibility=index==state.manifest.records.length-1?"hidden":"visible";$("complete").checked=item().complete;$("note").value=item().note;renderButtons();renderProgress();setMode(item().points[selected].visibility);setStatus("All changes autosave")}
